...
const Navbar = () => {
 ...
 document.body.style.overflow = 'auto'
  ...
  return (
    <>
      <div className="Nav h-20 flex gap-0 justify-between items-center">
        <div className=" sm:logo sm:flex sm:justify-center sm:items-center sm: h-18 sm: w-1/3  lg:pr-50">
          <Link href='/'><img src="./Image/logo.png" alt="" className='h-25 mt-2 md:mt-0 ' /></Link>
        </div>
        {/* //// */}
        ...
        {/* laptop bar */}
        <div className="navItem sm: hidden  md:flex justify-center items-end mr-2 mt-2  md:w-2/5 lg:text-4xl lg:justify-evenly ">
        ...
        {/* Mobile bar */}
        <div onClick={() => setshow(true)} className=" md:hidden bar justify-center flex items-center cursor-pointer mr-6 text-orange-500 mt-10 text-2xl"><FaBarsStaggered className='' /></div>
      ...
      <div className={` md:hidden ${show ? 'fixed w-full mr-6' : 'h-0 w-0'}  flex justify-end items-center lg:hidden `}>
        <div className="option flex  justify-end items-center gap-3 text-md pb-1 pr-5 pl-5 rounded-md mr-2 mt-6 font-semibold">
          <Link onClick={() => setshow(false)} href="/" className="">Home</Link>
    <Link onClick={() => setshow(false)} href="#About" className="">About</Link>
